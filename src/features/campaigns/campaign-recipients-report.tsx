...
import { downloadCsvFromApi, downloadTextCsv } from "@/features/data-io/download-csv";
...
export function CampaignRecipientsReport({
  ...
  const exportReport = async () => {
    setExporting(true);
    try {
      const filename = recipientReportFilename(campaignName, filter);
      if (!isPageMockMode()) {
      ...
      downloadTextCsv(filename, recipientsToCsv(items));
      toast.success(
        truncated
    ...
    <section className={cn(CARD_SURFACE_CLASS, "flex min-h-[420px] flex-col p-5")}>
      ...
      {errorCode != null ? (
        <p className="mt-3 text-sm text-muted-foreground">
          Filtrando falhas
          {errorCode === "other" ? " sem código da Meta" : ` · cód. ${errorCode}`}.
        </p>
      ) : null}
